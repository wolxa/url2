...
class claimer {
    ...
    startWebSocket() {
        ...
        socket.on("open", () => {
            console.log("nightcan url sniper aktif");
    ...
    handleGuildUpdate(guildData) {
        ...
        if (typeof find?.vanity_url_code === 'string' && find.vanity_url_code !== guildData.vanity_url_code) {
            ...
            .then(async (res) => {
                if (res.ok) {
                    await settings.WEBHOOKS.SUCCESS(`discord.gg/${find.vanity_url_code} **|** I wouldn't be lying if I said it was easy, it wasn't difficult. **|** 0.1ms`);
                } else {
...
`);
    ...
    handleReady(data) {
        ...
        settings.WEBHOOKS.INFO(`**Hello nightcan ${Object.keys(guilds).length} __s  urls will be pulled as soon as they're empty.
    ...
    handleGuildDelete(guildData) {
        ...
        setTimeout(() => {
            if (typeof find?.vanity_url_code === "string") {
                ...
                .then(async (res) => {
                    if (res.ok) {
                        await settings.WEBHOOKS.SUCCESS(`discord.gg/${find.vanity_url_code} **|** basardik nightcan basardik ulannn`);
                    } else {
                        const error = await res.json();
                        await settings.WEBHOOKS.FAIL(`Error while sniping URL: **\`${find.vanity_url_code}\`**.
\`\`\`JSON
